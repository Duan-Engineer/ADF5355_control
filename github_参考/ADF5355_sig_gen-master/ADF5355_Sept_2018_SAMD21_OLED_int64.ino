...
long Freq_Old;
long refin =  10000000;      // Referenzquarz = 100.000 Mhz - Enter exact value to calibrate
...
void SetFreq(long Frequ)     // Freq hier lokal oben global
{
  ...
  SerialUSB.begin(9600);    // USB to PC for Debug only
  SerialUSB.println(" ");
  SerialUSB.print("Frequuency = ");
  SerialUSB.println(Frequ);
...
////////////////// Set 32 bit register values R0 to R12 ///////////////////////////
 ...
 // R[6] = (unsigned long) (0x35002076);   //variable
  ...
  /////////////// Serial print values to terminal for diagnostics ///////////////
    ...
    SerialUSB.print("Inhalt R0 ");
    SerialUSB.println(R[0], HEX);
    SerialUSB.print("Inhalt R1 ");
    SerialUSB.println(R[1], HEX);
    SerialUSB.print("Inhalt R2 ");
    SerialUSB.println(R[2], HEX);
    SerialUSB.print("Inhalt R3 ");
...
void fixfrq_select()
{
  ...
  if (press == LOW)
  {
    ...
    if (cnt_fix == 0) {
      ...
      Freq = 14420000;  // 144.2 MHz  Bake
    }
    else if (cnt_fix == 3) {
      Freq = 43290000;  // 432.9 MHz   Bake
    }
    else if (cnt_fix == 4) {
      ...
      Freq = Freq = 340090000;  // 3400.9 MHz
    }
    else if (cnt_fix == 7) {
      Freq = 345630000;  // 3456.3 MHz Bake 3. OW auf 10368.9 MHZ
    }
    else if (cnt_fix == 8) {
      Freq = 576050000;  // 3435.5 MHz Bake 7. OW auf 24048.5 MHz
    }
    else if (cnt_fix == 9) {
      Freq = 1036810000;  // 4277.1  MHz Bake 11. OW auf 47048.1 MHz
    }
